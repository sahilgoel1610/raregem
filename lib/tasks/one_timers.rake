namespace :rare do
  ...
  task run_all: :environment do
  	last_onetimer_run = 0
		last_onetimer_run = TimeOneTimer.order(:creation_time).pluck(:creation_time).last.to_i
		Dir.chdir("lib/one_timers") do
			all_files = Dir["*.rb"]
			unran_files = all_files.select { |f| f[f.length - 17..f.length - 4].to_i  > last_onetimer_run }
